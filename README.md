...
We recommend the order from the largest to smallest, always saving any changes to original uploaded image for last.
 ...
 Each set of Resize instructions has three elements width, height, suffix. Each element is followed by a comma and each set is closed with a "|". To save an image with the original file name, leave the suffix element blank. You can also omit the closing "|" as the application will add it it not present. 
